...
public class ClienteService {
	...
	/*Uma exceção qualquer ocorra, nesse caso um rollback deverá ser feito 
	para o estado anterior (como se nenhuma das operações tivesse oc	orrido).*/
	@Transactional
	public Cliente salvar(Cliente cliente) {
		...
		if(clienteAux.isPresent()) {
			if(!clienteAux.get().equals(cliente)) {
				//esse if é para comparar se o cliente a "salvar" já é existente e está sendo apenas atualizado.
				//nesse caso ele sabe q os email são iguais e verifica se os IDs são os mesmos, caso nao retornamos a exception
